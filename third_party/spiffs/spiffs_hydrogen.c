...
s32_t SPIFFS_mount(spiffs *fs, spiffs_config *config, u8_t *work,
		u8_t *fd_space, u32_t fd_space_size, void *cache, u32_t cache_size,
...
#pragma GCC diagnostic pop
	...
	fs->cache = cache;
	fs->cache_size =
			(cache_size > (config->log_page_size * 32)) ?
					config->log_page_size * 32 : cache_size;
...
s32_t SPIFFS_creat(spiffs *fs, char *path, spiffs_mode mode) {
	(void) mode;
	SPIFFS_API_CHECK_CFG(fs);
	SPIFFS_API_CHECK_MOUNT(fs); SPIFFS_LOCK(fs);
	...
	SPIFFS_API_CHECK_RES_UNLOCK(fs, res); SPIFFS_UNLOCK(fs);
	return 0;
}

spiffs_file SPIFFS_open(spiffs *fs, char *path, spiffs_flags flags,
		spiffs_mode mode) {
	(void) mode;
	SPIFFS_API_CHECK_CFG(fs);
	SPIFFS_API_CHECK_MOUNT(fs); SPIFFS_LOCK(fs);
	...
	res = spiffs_object_find_object_index_header_by_name(fs, (u8_t*) path,
			&pix);
	...
	if ((flags & SPIFFS_CREAT) && res == SPIFFS_ERR_NOT_FOUND) {
		...
		SPIFFS_API_CHECK_RES_UNLOCK(fs, res);
		res = spiffs_object_create(fs, obj_id, (u8_t*) path, SPIFFS_TYPE_FILE,
				&pix);
		if (res < SPIFFS_OK) {
			spiffs_fd_return(fs, fd->file_nbr);
		}
...
static s32_t spiffs_hydro_write(spiffs *fs, spiffs_fd *fd, void *buf,
		u32_t offset, s32_t len) {
	(void) fs;
	s32_t res = SPIFFS_OK;
	s32_t remaining = len;
	...
	if ((fd->flags & SPIFFS_DIRECT) == 0) {
		if (len < (s32_t) SPIFFS_CFG_LOG_PAGE_SZ(fs)) {
				...
				if (offset < fd->cache_page->offset|| // writing before cache
					...
					res = spiffs_hydro_write(fs, fd,
							spiffs_get_cache_page(fs, spiffs_get_cache(fs),
									fd->cache_page->ix), fd->cache_page->offset,
							fd->cache_page->size);
			...
			if (fd->cache_page) {
				...
				u8_t *cpage_data = spiffs_get_cache_page(fs, cache,
						fd->cache_page->ix);
				memcpy(&cpage_data[offset_in_cpage], buf, len);
				fd->cache_page->size = MAX(fd->cache_page->size,
						offset_in_cpage + len);
				fd->fdoffset += len;
				SPIFFS_UNLOCK(fs);
				return len;
...
s32_t SPIFFS_lseek(spiffs *fs, spiffs_file fh, s32_t offs, int whence) {
	...
	if (fd->cursor_objix_spix != objix_spix) {
		...
		res = spiffs_obj_lu_find_id_and_span(fs,
				fd->obj_id | SPIFFS_OBJ_ID_IX_FLAG, objix_spix, 0, &pix);
		SPIFFS_API_CHECK_RES_UNLOCK(fs, res);
...
static s32_t spiffs_stat_pix(spiffs *fs, spiffs_page_ix pix, spiffs_file fh,
		spiffs_stat *s) {
	spiffs_page_object_ix_header objix_hdr;
	spiffs_obj_id obj_id;
	s32_t res = _spiffs_rd(fs, SPIFFS_OP_T_OBJ_IX | SPIFFS_OP_C_READ, fh,
			SPIFFS_PAGE_TO_PADDR(fs, pix), sizeof(spiffs_page_object_ix_header),
			(u8_t * )&objix_hdr);
	SPIFFS_API_CHECK_RES(fs, res);
	...
	res = _spiffs_rd(fs, SPIFFS_OP_T_OBJ_LU | SPIFFS_OP_C_READ, fh, obj_id_addr,
			sizeof(spiffs_obj_id), (u8_t * )&obj_id);
	SPIFFS_API_CHECK_RES(fs, res);
...
s32_t SPIFFS_rename(spiffs *fs, char *old, char *new) {
	...
	s32_t res = spiffs_object_find_object_index_header_by_name(fs, (u8_t*) old,
			&pix_old);
	SPIFFS_API_CHECK_RES_UNLOCK(fs, res);

	res = spiffs_object_find_object_index_header_by_name(fs, (u8_t*) new,
			&pix_dummy);
	...
	res = spiffs_object_update_index_hdr(fs, fd, fd->obj_id, fd->objix_hdr_pix,
			0, (u8_t*) new, 0, &pix_dummy);
...
static s32_t spiffs_read_dir_v(spiffs *fs, spiffs_obj_id obj_id,
		spiffs_block_ix bix, int ix_entry, u32_t user_data, void *user_p) {
	(void) user_data;
	s32_t res;
	spiffs_page_object_ix_header objix_hdr;
	...
	if (res != SPIFFS_OK)
		return res;
	if ((obj_id & SPIFFS_OBJ_ID_IX_FLAG) && objix_hdr.p_hdr.span_ix == 0
			&& (objix_hdr.p_hdr.flags
					& (SPIFFS_PH_FLAG_DELET | SPIFFS_PH_FLAG_FINAL
...
struct spiffs_dirent *SPIFFS_readdir(spiffs_DIR *d, struct spiffs_dirent *e) {
	...
	res = spiffs_obj_lu_find_entry_visitor(d->fs, d->block, d->entry,
...
s32_t SPIFFS_info(spiffs *fs, u32_t *total, u32_t *used) {
	...
	u32_t total_data_pages = (blocks - 2) * (pages_per_block - obj_lu_pages)
