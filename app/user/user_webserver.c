...
#include "../../include/espressif/c_types.h"
#include "../../include/espressif/esp_libc.h"
#include "../../include/espressif/esp_misc.h"
#include "../../include/espressif/esp_softap.h"
#include "../../include/espressif/esp_sta.h"
#include "../../include/espressif/esp_system.h"
#include "../../include/espressif/esp_timer.h"
#include "../../include/espressif/esp_wifi.h"
#include "../../include/espressif/queue.h"
#include "../../include/freertos/portmacro.h"
#include "../../include/freertos/projdefs.h"
#include "../../include/lwip/ipv4/lwip/inet.h"
#include "../../include/lwip/ipv4/lwip/ip4_addr.h"
#include "../../include/lwip/lwip/def.h"
#include "../../include/lwip/lwip/multi-threads/sockets_mt.h"
#include "../../include/lwip/lwip/sys.h"
...
//LOCAL char *content =
//		"HTTP/1.1 200 OK\r\nContent-Type: text/html\r\nContent-Length:244\r\n\r\n"
//				"<html><span style=\"white-space:pre\"></span><title>LOGIN</title><body><form method=\"get\" action='WiFi.cgi'>SSID:<input type='text' name='ssid'><br>密码:<input type='password' name='password'><br><input type='submit'value='确认'></form></body></html>";
...
LOCAL int connect_status_get(cJSON *pcjson, const char* pname) {
	...
	cJSON_AddNumberToObject(pSubJson_Status, "status",
			user_esp_platform_get_connect_status());
...
LOCAL int wifi_info_get(cJSON *pcjson, const char* pname) {
	...
	if (wifi_station_get(pSubJson_Station) == -1)
		return -1;
	if (wifi_softap_get(pSubJson_Softap) == -1)
		return -1;
 ...
 *******************************************************************************/
LOCAL int wifi_req_parse(char *pValue) {
	...
	if ((pJsonSub = cJSON_GetObjectItem(pJsonSub_request, "Softap")) != NULL) {
		...
		if (NULL != pJsonSub_Sub) {
			WS_DEBUG(
					"pJsonSub_Connect_Softap pJsonSub_Sub->password %s  len%d\n",
					pJsonSub_Sub->valuestring,
					strlen(pJsonSub_Sub->valuestring))
			;
 ...
 * Returns      : none
 *******************************************************************************/
LOCAL void parse_url(char *precv, URL_Frame *purl_frame) {
	char *str = NULL;
	uint8 length = 0;
	char *pbuffer = NULL;
	char *pbufer = NULL;
	...
	if (ptemp != NULL) {
		...
		if (pdata != NULL) {
			...
			if (tmp_precvbuffer != NULL) {
				WS_DEBUG(" tmp_precvbuffer 0x%x pdata 0x%x ", tmp_precvbuffer,
						pdata)
				;
 ...
 * Returns      : none
 *******************************************************************************/
LOCAL void restart_10ms_cb(void *arg) // Jeremy.L
{
 ...
 * Returns      :
 *******************************************************************************/
LOCAL void data_send(struct single_conn_param *psingle_conn_param,
bool responseOK, char *psend) {
	uint16 length = 0;
	char *pbuf = NULL;
 ...
 * Returns      : none
 *******************************************************************************/
LOCAL void json_send(struct single_conn_param *psingle_conn_param,
		ParmType ParmType) {
	char *pbuf;
	cJSON *pcjson;
	int ret = 0;
 ...
 * Returns      : none
 *******************************************************************************/
LOCAL void response_send(struct single_conn_param *psingle_conn_param,
bool responseOK) {
...
LOCAL void json_scan_cb(void *arg, STATUS status) {
	...
	error_handle: cJSON_Delete(pJson);
	if (pbuf != NULL)
		free(pbuf);
 ...
 * Returns      : none
 *******************************************************************************/
void upgrade_check_func(struct single_conn_param *psingle_conn_param) {
 ...
 * Returns      : none
 *******************************************************************************/
LOCAL void local_upgrade_deinit(void) {
	if (system_upgrade_flag_check() != UPGRADE_FLAG_START) {
 ...
 * Returns      : none
 *******************************************************************************/
LOCAL void local_upgrade_download(struct single_conn_param *psingle_conn_param,
 ...
 * Returns      : none
 *******************************************************************************/
LOCAL void webserver_recvdata_process(
		struct single_conn_param *psingle_conn_param, char *pusrdata,
...
//	printf("pURL_Frame->Type:\n%d\n", pURL_Frame->Type);
//	printf("pURL_Frame->pSelect:\n%s\n", pURL_Frame->pSelect);
//	printf("pURL_Frame->pCommand:\n%s\n", pURL_Frame->pCommand);
//	printf("pURL_Frame->pFilename:\n%s\n", pURL_Frame->pFilename);

//	if (pURL_Frame->Type != 0 && pURL_Frame->Type != 1) {
////		single_conn_param->sock_fd
//		write(psingle_conn_param->sock_fd, content, strlen(content));
//	}

	if (strstr(precvbuffer, "GET") != NULL) {
		//GET
		if (strstr(precvbuffer, "WiFi.cgi") != NULL) {
			if (strstr(precvbuffer, "?") != NULL) {
				char *src = precvbuffer;
				char *dest = zalloc(100);
				char *p1, *p2;
				p1 = strstr(src, "?");
				p2 = strstr(src, " HTTP");
				if (p1 == NULL || p2 == NULL || p1 > p2) {
					...
					if (saveSSIDAndPassword(ssid, password) == 0) {
						printf("succed \n");
						write(psingle_conn_param->sock_fd, content_succed,
								strlen(content_succed));
					} else {
						printf("failed \n");
						write(psingle_conn_param->sock_fd, content_failed,
								strlen(content_failed));
					}
			...
			write(psingle_conn_param->sock_fd, wifi_configure_all,
					strlen(wifi_configure_all));
		}
	} else if (strstr(precvbuffer, "POST") != NULL) {

	}

	temp_exit: if (precvbuffer != NULL) {
		free(precvbuffer);
		precvbuffer = NULL;
	}
	...
	while (1) {
		...
		if (ret > 0) {
			...
			for (index = 0; index < MAX_CLIENT_NUMBER; index++) {
				...
				if (FD_ISSET(pconnections->single_conn[index]->sock_fd,
						&readset)) {
					/*stop the sock handle watchout timer */
					os_timer_disarm(
							(os_timer_t *) &pconnections->single_conn[index]->stop_watch);
					memset(precvbuf, 0, RECV_BUF_SIZE);

					ret = recv(pconnections->single_conn[index]->sock_fd,
							precvbuf, RECV_BUF_SIZE, 0);
					if (ret > 0) {
						/*
						...
						;
						webserver_recvdata_process(
								pconnections->single_conn[index], precvbuf,
								ret);

						/*restart the sock handle watchout timer */
						os_timer_setfn(
								(os_timer_t *) &pconnections->single_conn[index]->stop_watch,
								(os_timer_func_t *) webserver_conn_watcher,
								pconnections->single_conn[index]);
						os_timer_arm(
								(os_timer_t *) &pconnections->single_conn[index]->stop_watch,
								STOP_TIMER, 0);
...
//			;
	...
	for (index = 0; index < MAX_CLIENT_NUMBER && pconnections->conn_num;
			index++) {
		//find all valid handle
		if (pconnections->single_conn[index]->sock_fd >= 0) {
			os_timer_disarm(
...
void webserver_recv_task_start(struct conn_param* pconnections) {
	...
	if (RCVQueueStop != NULL) {
		sys_thread_new("websrecv_thread", webserver_recv_thread, pconnections,
				512, 6);  //1024, 704 left 320 used
	}
}
...
LOCAL multi_conn_init(void) {
	...
	for (index = 0; index < MAX_CLIENT_NUMBER; index++) {
		single_conn[index] = (struct single_conn_param *) zalloc(
				sizeof(struct single_conn_param));
 ...
 * Returns      : none
 *******************************************************************************/
LOCAL void user_webserver_task(void *pvParameters) {
	int32 listenfd;
	int32 remotefd;
	int32 len;
	int32 ret;
	...
	while (1) {
		...
		if (remotefd != -1) {
			...
			if (index < MAX_CLIENT_NUMBER) {
				...
				os_timer_setfn(&pconnections->single_conn[index]->stop_watch,
						(os_timer_func_t *) webserver_conn_watcher,
						pconnections->single_conn[index]);
				os_timer_arm(&pconnections->single_conn[index]->stop_watch,
				STOP_TIMER, 0);
